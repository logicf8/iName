...
export function artValuesModifyComb(sectionPortfolio, header){
   header.returnArticles().forEach(article => {
    switch(article.group1){
      case G1[0]: { combinationTypeAndSize(sectionPortfolio, header, article) } break;
      case G1[1]: { typeOfAppliance(sectionPortfolio, header, article); } break;
      case G1[2]: { if(article.group2 !== "Tillbehör"){header.forFlags.sink = true; sectionPortfolio.cmInfoFlag.posCutOut += 1; } } break;
      case G1[3]: { interior(header, article); } break;
      case G1[4]: { fronts(header, article) } break;
      case G1[5]: { frontAcc(header, article)} break;
    }
  });
}
...
function frontAcc(header, article){
  switch(article.group2){
    ...
    case G1_5_G2[1]: 
    {
      if(article.group3 === G1_5_G2_1[0]){ //"LådaPåDörr"
      ...
      else if(article.group3 === G1_5_G2_1[2]){
...
function interior(header, article){
  switch(article.group2) {
    ...
    case G1_3_G2[1]: { let x = 1; article.group3 === G1_3_G2_0[0] ? x = 1 : x = 2;  header.withFlags.shelfs += article.quantity * x;} break;
...
function fronts(header, article)
{
  switch (article.group2){
    ...
    case G1_4_G2[1]: { 
      let x = 1; article.height === 10 ? x = 2 : x = 1; header.drawersFlags.drawerFront += article.quantity * x 
    }
  }
}
function typeOfAppliance(sectionPortfolio, header, article){
  switch(article.group2){
    ...
    case G1_1[5]: { header.forFlags.fridgeOrF = true; header.applianceDescription === undefined ? header.applianceDescription = article.description : header.applianceDescription = "kyl och fys";} break;
  }  
}
...
function combinationTypeAndSize(sectionPortfolio, header, article){
  ...
  if(header.description === undefined || article.description === G1_0[2]){
    if(article.group2 === G1_0[3] && (header.originalTxt.includes("METOD - Base cabinet") || header.originalTxt.includes("METOD - Bänkskåp"))){
      header.description = `Bänkskåp ${article.width} cm av överskåp`; type = "Bänkskåp"
    }
    else if(article.group2 === G1_0[0] && header.originalTxt.includes("Bänkhörnskåp 135 grader")){
      header.description = `Bänkhörnskåp ${article.width} cm`; header.corner135 = true; type = "Bänkskåp";
    }
    else{
